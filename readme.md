...
```
A Replica is replicated from the server to all clients. Replicas must always be prefabs and instances of prefabs for Cube to be able to create client instances of them.
```
...
```
A ReplicaView observes Replicas for a connection. Its position is used to priorize which Replicas to send.
```
